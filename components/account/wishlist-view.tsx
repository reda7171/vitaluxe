                ...
                {products.length > 0 && (
                    <div className="pt-4 border-t">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {ALL_PRODUCTS.filter((p) => !wishlist.some(w => w.productId === String(p.id)))
                                .slice(0, 4)
                                .map((product) => (
                                    <Link
                                        key={product.id}
                                    ...
                                    >
                                        ...
                                        <p className="text-sm font-extrabold mt-1">{product.salePrice ?? product.price} Dhs</p>
                                        <button
                                            onClick={async (e) => {
                                                e.preventDefault();
                                                // Optimistic update
                                                const optimisticEntry = { id: `temp-${product.id}`, productId: String(product.id), product: product };
                                            ...
                                            className="mt-2 w-full flex items-center justify-center gap-1.5 text-xs text-muted-foreground hover:text-rose-500 transition-colors border rounded-lg py-1.5"
                                        >
                                            <Heart className="h-3 w-3" /> Ajouter aux favoris
                                        </button>
                                    </Link>
                                ))}
                        </div>
