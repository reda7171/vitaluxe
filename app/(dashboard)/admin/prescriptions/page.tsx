...
export default function AdminPrescriptionsPage() {
    ...
    const remove = async (id: string) => {
        ...
        if (res.ok) {
            toast.success("Ordonnance supprimée", { id: toastId });
    ...
    const removeMultiple = async () => {
        if (!confirm(`Supprimer les ${selectedIds.length} ordonnances sélectionnées ?`)) return;
        const toastId = toast.loading("Suppression multiple...");
        try {
            const res = await fetch("/api/admin/prescriptions/bulk", {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ ids: selectedIds })
            });
            if (res.ok) {
                toast.success("Ordonnances supprimées", { id: toastId });
                setSelectedIds([]);
                load();
            } else {
    ...
    const toggleSelectAll = () => {
        if (selectedIds.length === prescriptions.length && prescriptions.length > 0) {
            setSelectedIds([]);
        } else {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                {selectedIds.length > 0 && (
                    <button onClick={removeMultiple}
                        className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-all shadow-md shadow-red-600/20">
                    ...
                    <table className="w-full text-left min-w-[700px]">
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                return (
                                    <tr key={p.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${selectedIds.includes(p.id) ? "bg-[#103178]/5" : ""}`}>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="rounded-xl overflow-hidden border border-slate-200 bg-slate-50 flex items-center justify-center">
                                {preview.image.startsWith("data:image") ? (
                                    <img src={preview.image} alt="Prescription" className="w-full h-auto" />
                                ) : (
